...
public class UserMapper {
    public static User login(String email, String password, ConnectionPool connectionPool) throws DatabaseException {
        String sql = "select * from users INNER JOIN address ON users.addressID = address.addressID INNER JOIN orders ON users.userID = orders.userID WHERE email = ? AND password = ?";
    ...
    public static List<User> getAllUsers(ConnectionPool connectionPool) throws DatabaseException {
        ...
        String sql = "SELECT * FROM users INNER JOIN address ON users.addressID = address.addressID INNER JOIN orders ON users.userID = orders.userID";
        try (Connection connection = connectionPool.getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                int userId = rs.getInt("userID");
